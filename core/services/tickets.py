...
def register_ticket(event_id, first_name, last_name, email, seat, idempotency_key=None):
    ...
    if idempotency_key:
        record = TicketRequestIdempotency.objects.filter(
            idempotency_key=idempotency_key
        ).first()
        if record:
            same_request = (
                record.event_id == event_id
                and record.first_name == first_name
                and record.last_name == last_name
                and record.email == email
                and record.seat == seat
            )
    ...
    ticket = Ticket.objects.filter(
        event=event, seat=seat, canceled_at__isnull=True
    ).first()
    if ticket:
    ...
    with transaction.atomic():
        try:
            Ticket.objects.create(
                event=event, ticket_id=ticket_id, seat=seat, email=email
            )
        ...
        NotificationOutbox.objects.create(
            event_type="ticket_purchased", payload=payload
        )
        if idempotency_key:
            TicketRequestIdempotency.objects.create(
                event=event,
                ticket_id=ticket_id,
                seat=seat,
                email=email,
...
def unregister_ticket(ticket_id):
    ...
    ticket = (
        Ticket.objects.filter(ticket_id=ticket_id, canceled_at__isnull=True)
        .order_by("-created_at")
        .first()
    )
